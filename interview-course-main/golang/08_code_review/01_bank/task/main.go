// Задача: Сделайте ревью кода
//
// ТЗ: собирать информацию по курсам валют в разных банках.
// Программа каждую минуту отправляет запрос в банк и получает курс валют,
// затем сохраняет результат в БД. Банков может быть несколько.
//
// Требования:
// - Желательно корректно завершать работу с graceful shutdown
// - Найдите все проблемы в коде
		...
		if cmdName == "help" {
			...
			for _, url := range urlsBank {
				...
				if url.bankName == "Bank 1" {
					// Заменяем для Банка 1 запятую на точку
				...
				if err != nil {
					// не хорошо что наше приложение может в любой момент упасть
					panic(err)
				}
			}
		}
